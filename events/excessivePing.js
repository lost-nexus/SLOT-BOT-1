    ...
    async execute(message) {
        ...
        if (message.content.toLowerCase().includes("@everyone")) {
            ...
            if (!count) {
                ...
                message.channel.send({embeds: [{description: `You have used 1/2 pings`, color: 0xff0000}]})
            } else if (count === 1) {
                await db.set(`count.${message.channel.id}`, 2)
                message.channel.send({embeds: [{description: `You have used 2/2 pings`, color: 0xff0000}]})
            } else if (count === 2) {
                await message.channel.permissionOverwrites.edit(message.author.id, { SendMessages: false });
                ...
                client.giveawaysManager
                    .pause(onServer[0].messageId)
                    .then(() => {
                        message.reply({embeds: [{title: `Slot has been paused`, description: `Reason: Use of More than 2 pings`, color: 0xff0000}]});
                    })
                    .catch((err) => {
                        message.reply({ephemeral: true, content: `An error has occurred, please check and try again.\n\`${err}\``});
                });
